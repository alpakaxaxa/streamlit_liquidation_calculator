...
with st.form(key='liquidation_information'):
    year_input = st.number_input(
        'Steuerjahr', min_value=2019, max_value=2022, value=CURRENT_YEAR-1, step=1)
    municipality = st.selectbox('Gemeinde', municipalities)
    married = st.checkbox('Verheiratet', key='married')
    denomination = st.selectbox(
        'Konfession', ('Konfessionslos', 'Römisch-Katholisch', 'Evangelisch'), key='denomination')
    denomination_partner = st.selectbox('Konfession Ehepartner', (
        'Konfessionslos', 'Römisch-Katholisch', 'Evangelisch'), key='denomination_partner')
...
if submit_button:
    data = get_tax_data()
    simple_tax = calculate_simple_tax(
        married, notional_purchase, other_liquidation_profit)
    municipality_data = find_municipality_data(data, municipality)
    simple_tax_multiplier = extract_simple_tax_multiplier(
        municipality_data, married, denomination, denomination_partner)
    local_tax = simple_tax * simple_tax_multiplier

    federal_tax_other_liquidation_profit = calculate_federal_tax(
        other_liquidation_profit/5, married)
    ...
    federal_tax_other_liquidation_profit_rate = federal_tax_other_liquidation_profit / \
        (other_liquidation_profit_divider/5)

    if federal_tax_other_liquidation_profit_rate < 0.02:
        federal_tax_other_liquidation_profit_rate = 0.02

    federal_tax_other_liquidation_profit = other_liquidation_profit * \
        federal_tax_other_liquidation_profit_rate
    federal_tax_notional_purchase = calculate_federal_tax(
        notional_purchase, married)/5
